...
- Same-morning rules and features are versioned via `scripts/lib/versions.mjs` (`FEATURE_VERSION_V1`/`RULE_VERSION_V1` frozen baseline, `FEATURE_VERSION_V2`/`RULE_VERSION_V2` candidates). `scripts/lib/call-rule.mjs` (v1) is frozen and must not change; new candidate logic goes in `scripts/lib/call-rule-v2.mjs` (or a later version) and must not overwrite v1. `scripts/backtest-katabatic.mjs` generates paired v1/v2 rows and `scripts/score-backtest.mjs --rule-version <id>` scores one version at a time. A candidate only replaces the live rule when it clears the promotion bar documented in `research/katabatic-prediction.md` §7.1a; an inconclusive/negative result is recorded there and is not shipped.
...
- The live skill script (`.github/skills/dp-katabatic-check/scripts/katabatic-check.mjs`) uses the same shared `computeFeatures`/`callRule` (v1) as the backtest for its actual verdict, `computeFeaturesV2` only for descriptive trajectory text, and persists every call via `prediction-log-store.mjs` — keep it wired to the shared libs rather than reimplementing feature logic inline.
