...
- `scripts/katabatic-refresh.mjs` orchestrates the perishable-data path: drain/capture Holfuy first, capture Ecowitt, relabel/backtest, score, and run the non-decision Lookout analysis. `.github/workflows/archive-weather-observations.yml` runs this daily and creates a verified Postgres dump artifact.
- `scripts/archive-hrrr-forecast.mjs` captures the exact 00Z HRRR run for a morning. `scripts/lib/night-before-call.mjs` contains the frozen experimental rule/model, while `scripts/lib/night-before-prediction-store.mjs` materializes versioned predictions. `.github/workflows/collect-night-before-forecast.yml` runs the forward collector and a later recovery attempt.
- `cloudflare/workflow-scheduler/` owns every clock-based trigger. Its single quarter-hour Cloudflare cron admits explicit `America/Denver` times and invokes the GitHub workflows through `workflow_dispatch`; do not reintroduce GitHub `schedule` blocks.
