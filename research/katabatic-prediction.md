...
As of 2026-09-19, one Cloudflare Cron Trigger dispatches two GitHub Actions **collection** jobs:

1. At 9:00 and 9:15 p.m. Denver time, `collect-night-before-forecast.yml` stores the exact 00Z
   HRRR run's raw 05:00–08:00 wind and boundary-layer-height values in Neon. It then writes an
   immutable, versioned experimental call and success chance to `night_before_predictions`. It
   does **not** run `/dp-katabatic-check`.
2. At 2:15 p.m. Denver time the next afternoon, `archive-weather-observations.yml` stores the
   station observations, derives the rideable/not-rideable outcome, and attaches a verified
   database dump. Together they produce one forecast/outcome pair.

GitHub has no `schedule` triggers for these jobs. The Cloudflare Worker converts its quarter-hour
UTC trigger to `America/Denver` and calls `workflow_dispatch`; the workflows must therefore exist
...
- **Retries with backoff.** The 00Z run publishes ~00:50 UTC. The primary collector runs at
  9:00 p.m. Denver time and Cloudflare dispatches the idempotent workflow again at 9:15; transient
  errors within either run back off and retry, while a genuine *"model run is not available"*
...
⚠️ **Dispatch requires the default branch.**
`.github/workflows/collect-night-before-forecast.yml` is invoked by the Cloudflare scheduler
through GitHub's `workflow_dispatch` API, which only accepts a workflow definition present on the
repository's default branch. A scheduler or deployment failure must be repaired or manually
dispatched; **every day it is not run is a morning that can never be recovered.**
